...
def pending_verification_mail(sender, user: User, signal_type: SignalType, **kwargs) -> int:
    is_an_admin = user.is_superuser or user.role == user.ROLES.ADMIN
    is_active_and_verified = user.is_verified and user.is_active

    if is_an_admin or is_active_and_verified or signal_type != SignalType.PENDING:
        return None
